...
你在服务器上手动执行 update.sh（docker compose pull && up -d）
  · 拉取最新镜像、重启容器、自动同步数据库结构（prisma db push）、健康检查
```

- 镜像名里的 `<owner>/<repo>` 就是你的 GitHub 仓库（如仓库是 `github.com/acme/xunjian`，
  镜像即 `ghcr.io/acme/xunjian-server`、`ghcr.io/acme/xunjian-admin`）。
- 数据库结构同步（Prisma `db push`）在后端容器启动时自动执行，无需手动操作。
  它会把库结构对齐到 `schema.prisma`：缺表建表、缺列补列，加法改动安全；遇到会丢数据的
  破坏性改动会直接报错退出（不会误删生产数据）。
...
`update.sh` 会：拉取最新镜像 → 启动 MySQL/Redis/后端/nginx → 后端启动时自动同步数据库结构 →
...
即可拉取新镜像、滚动重启、自动同步数据库结构、健康检查。前端或后端任一改动都会各自触发对应镜像构建。
