  ...
  private buildRouteTree(
    ...
    return items
      ...
      .map((item) => {
        ...
        // - 顶级单页菜单(type1 且无父目录)：用完整 router 作根（/category），作为一级菜单由前端套 Layout
        // - 子级菜单(type1 有父目录)：用末段相对片段（department），前端与父路径拼接还原完整路径
        const path =
          item.type === 0 || (item.type === 1 && item.parentId == null)
            ? router
